...
def _pl_load_term_details(term: int) -> dict:
    ...
    detail_dir = os.path.join(DATA_DIR, "case_detail", str(term))
    result = {}
    if os.path.isdir(detail_dir):
        for path in _glob.glob(os.path.join(detail_dir, "*.json")):
            try:
...
def _pl_load_vote_data(terms: tuple) -> list[dict]:
    ...
    for term in terms:
        ...
        term_details = _pl_load_term_details(term)
        for c in cases:
            href = c.get("href", "")
            if not href: continue
            detail = term_details.get(href) or _pl_fetch_detail(href)
        ...
        if not vote_rows_data:
            ...
            for pres in (pres_sel_ca or df_votes_filtered["president"].unique()):
                ...
                with st.expander(f"**{pres}** ({len(pres_df):,} votes, {pres_df['justice'].nunique()} justices)",
                    ...
                    st.dataframe(j_df.reset_index(drop=True), height=200)
