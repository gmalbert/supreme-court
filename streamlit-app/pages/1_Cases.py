...
st.set_page_config(page_title="Cases Hub", page_icon="⚖️", layout="wide")
...
with tab_oral:
    ...
    oa_term = st.selectbox("Select Term", list(range(CURRENT_YEAR, CURRENT_YEAR - 32, -1)), key="oa_term")
    ...
    if not oa_cases:
        ...
        oa_names = [c.get("name", "Unknown") for c in oa_cases]
        ...
        if oa_selected:
            ...
            if not oa_detail:
                ...
                with col_side:
                    ...
                    disposition = oa_detail.get("disposition") or {}
                ...
                if not oral_args:
                    ...
                    for i, arg in enumerate(oral_args):
                        ...
                        with st.expander(f"**{title}**", expanded=(i == 0)):
                            if arg_href:
                                st.markdown(f"[🔗 Open on Oyez]({arg_href})")
...
with tab_calendar:
    ...
    STATUS_COLOR = {"Decided":"#27AE60","Argued":"#3498DB","Scheduled":"#E67E22","Cert Granted":"#9B59B6","Unknown":"#95A5A6"}
    STATUS_ICON  = {"Decided":"✅","Argued":"🔵","Scheduled":"📅","Cert Granted":"📌","Unknown":"❓"}
    ...
    def _extract_arg_date(oral_args):
        months = {m[:3].lower(): i for i, m in enumerate(
            ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"], 1)}
        for arg in (oral_args or []):
            if not isinstance(arg, dict): continue
            m = re.search(r'(\w+)\s+(\d{1,2}),?\s+(\d{4})', arg.get("title", ""))
            if m:
                mon = months.get(m.group(1)[:3].lower())
                if mon:
                    try: return datetime.date(int(m.group(3)), mon, int(m.group(2)))
                    except ValueError: pass
        return None

    def _case_status(detail, arg_date, decided_date):
        if decided_date: return "Decided"
        if arg_date and arg_date <= TODAY: return "Argued"
        if arg_date and arg_date > TODAY: return "Scheduled"
        if detail.get("decisions") or []: return "Decided"
        if detail.get("oral_argument_audio") or []: return "Argued"
        return "Cert Granted"
    ...
    if not cal_summary:
        ...
        st.info(f"Found **{len(cal_summary)}** cases. Loading details for dates…")
        ...
        for i, c in enumerate(cal_summary):
            ...
            if detail:
                decided_date = _parse_unix(detail.get("decided_on"))
                oral_args    = detail.get("oral_argument_audio") or []
                arg_date     = _extract_arg_date(oral_args)
                ...
                cal_data.append({
                    ...
                    "issue_area": issue, "status": _case_status(detail, arg_date, decided_date),
                ...
                cal_data.append({"name": c.get("name",""), "docket": c.get("docket_number",""),
                                  "issue_area": "Unknown", "status": "Unknown", "arg_date": None,
                                  "decided_date": None, "disposition": "", "vote_split": "", "href": href, "has_audio": False})
        ...
        with sub_issue:
            ...
            status_cols = [s for s in ["Decided","Argued","Scheduled","Cert Granted","Unknown"] if s in issue_df_cal.columns]
            ...
            st.plotly_chart(fig_issue_cal, use_container_width=True)
