    ...
    if "drift_rows" not in st.session_state:
        ...
        if not drift_rows:
            ...
            if drift_summary:
                ...
                st.dataframe(ds_df.reset_index(drop=True), use_container_width=True, height=260, hide_index=True)
...
with tab_doctrine:
    ...
    fig_all_doc.update_layout(barmode="group",title="Doctrinal Direction Across All Tracked Doctrines",
                               ...
                               legend=dict(x=1.01,y=1))
    st.plotly_chart(fig_all_doc, use_container_width=True)
...
with tab_congress:
    ...
    if not tl_df_cr.empty:
        ...
        st.plotly_chart(fig_tl_cr, use_container_width=True)
