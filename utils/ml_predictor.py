...
# ── Disposition parsing ───────────────────────────────────────────────────────
def _infer_outcome(detail: dict) -> int | None:
    """
    Infer affirm (0) vs reverse (1) from winning_party vs case name.
    Petitioner = first party in "A v. B"; winning petitioner → likely reverse.
    Falls back to None when indeterminate.
    """
    decs = detail.get("decisions") or []
    dec = decs[0] if decs else {}
    winner = (dec.get("winning_party") or "").strip().lower()
    if not winner:
        return None
    ...
    if len(parts) < 2:
        return None
    petitioner = parts[0].strip().lower()
    respondent = parts[1].strip().lower()
    # Check if winner substring matches petitioner or respondent
    def _matches(party: str, target: str) -> bool:
        words = [w for w in re.split(r'\W+', target) if len(w) > 2]
        return any(w in party for w in words) or any(w in target for w in re.split(r'\W+', party) if len(w) > 2)
    pet_match = _matches(winner, petitioner)
    res_match = _matches(winner, respondent)
    if pet_match and not res_match:
        return 1  # petitioner won → lower court reversed
    if res_match and not pet_match:
        return 0  # respondent won → lower court affirmed
    return None
...
def _load_term_details_local(term: int) -> dict:
    """Batch-load all local detail files for a term. Returns {href: detail}."""
    detail_dir = os.path.join(_LOCAL_DATA_DIR, "case_detail", str(term))
    result = {}
...
def collect_training_data(
    ...
    """
    Load case + vote data for given terms from local cache.
    ...
    for ti, term in enumerate(needed):
        if stop_event and stop_event.is_set():
            break
        if progress_cb:
            progress_cb(ti, len(needed), f"Loading {term}–{term+1} term cases…")
        ...
        for ci, c in enumerate(cases):
            ...
            if progress_cb and ci % 10 == 0:
                progress_cb(ti, len(needed),
                            f"Term {term}: case {ci+1}/{n_cases} — {len(new_rows)} rows so far")
            ...
            outcome = _infer_outcome(detail)
            if outcome is None:
                continue  # skip indeterminate cases
...
def train_models(
    ...
    # cv=5 cross-validated calibration on training data (cv="prefit" removed in newer sklearn)
    cal_outcome = CalibratedClassifierCV(outcome_pipeline, method="sigmoid", cv=5)
    cal_outcome.fit(X_train, yo_train)
    ...
    cal_split = CalibratedClassifierCV(split_pipeline, method="sigmoid", cv=5)
    cal_split.fit(X_train, ys_train)
    ...
    for justice in CURRENT_JUSTICES:
        ...
        if len(Xj_test) >= 10:
            cal_j = CalibratedClassifierCV(j_pipeline, method="sigmoid", cv=5)
            cal_j.fit(Xj_train, yj_train)
    ...
    try:
        # With cv=5, fitted sub-pipelines live in calibrated_classifiers_[i].estimator
        inner_pipeline = cal_outcome.calibrated_classifiers_[0].estimator
